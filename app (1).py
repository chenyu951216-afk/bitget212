...
def _ai_strategy_profile(symbol, regime='neutral', setup=''):
    ...
    try:
        ...
        if int(learn_srs.get('count', 0) or 0) > 0:
            ...
            profile.update({
                'ready': int(exact_row.get('count', 0) or 0) >= 6,
        ...
        if cnt >= 5:
            profile['ready'] = True
            if cnt >= 10 and (wr < 48 or avg < -0.15):
                profile['hard_block'] = True
                profile['threshold_adjust'] = 999
                profile['note'] = 'AI判定此策略偏弱，暫停自動下單'
            elif wr >= 60 and avg > 0.1:
                bonus = min(5.0, 1.0 + (wr - 58) / 8.0 + min(avg, 1.5) * 0.8)
                profile['threshold_adjust'] = -round(bonus, 2)
                profile['note'] = 'AI判定策略偏強，小幅放寬門檻'
            elif wr < 52 or avg < 0:
                malus = min(6.0, 1.0 + max(0, 54 - wr) / 6.0 + abs(min(avg, 0)) * 1.0)
                profile['threshold_adjust'] = round(malus, 2)
                profile['note'] = 'AI判定策略偏弱，提高門檻保守處理'
            else:
                profile['threshold_adjust'] = 0.0
                profile['note'] = 'AI判定中性，沿用主策略'
    except Exception as e:
...
def run_multi_market_backtest(symbols=None):
    ...
    results.sort(key=lambda x: (x.get('return_pct', 0), x.get('profit_factor') or 0, x.get('win_rate', 0), x.get('trades', 0)), reverse=True)
