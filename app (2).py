...
def _ai_strategy_profile(symbol, regime='neutral', setup=''):
    profile = {
        ...
        'hard_block': False,
        'strategy': f'{regime}|{setup}|{symbol}',
        'note': 'AI樣本不足，沿用主策略',
    }
    try:
        ...
        with AI_LOCK:
            db = AI_DB
            board = list(db.get('strategy_scoreboard', []) or [])
            sr = dict((db.get('symbol_regime_stats', {}) or {}).get(f'{symbol}|{regime}', {}) or {})
            market_rows = list((AUTO_BACKTEST_STATE.get('results') or []))
        exact_row = next((r for r in board if str(r.get('strategy')) == strategy_key), None)
        if exact_row:
            profile.update({
                'ready': int(exact_row.get('count', 0) or 0) >= 4,
                'source': 'strategy_scoreboard',
                'sample_count': int(exact_row.get('count', 0) or 0),
                'win_rate': _normalize_wr_percent(exact_row.get('win_rate', 0)),
                'avg_pnl': float(exact_row.get('avg_pnl', 0) or 0),
                'strategy': str(exact_row.get('strategy') or strategy_key),
            })
        elif int(sr.get('count', 0) or 0) > 0:
            cnt = int(sr.get('count', 0) or 0)
            wr = (float(sr.get('win', 0) or 0) / max(cnt, 1)) * 100.0
            avg = float(sr.get('pnl_sum', 0) or 0) / max(cnt, 1)
            profile.update({
                'ready': cnt >= 4,
                'source': 'symbol_regime_stats',
                'sample_count': cnt,
                'win_rate': wr,
                'avg_pnl': avg,
            })
        else:
            market_row = next((r for r in market_rows if str(r.get('symbol')) == symbol), None)
            if market_row:
                cnt = int(market_row.get('trades', 0) or 0)
                profile.update({
                    'ready': cnt >= 4,
                    'source': 'market_backtest',
                    'sample_count': cnt,
                    'win_rate': _normalize_wr_percent(market_row.get('win_rate', 0)),
                    'avg_pnl': float(market_row.get('return_pct', 0) or 0),
                    'strategy': f"{symbol}|{market_row.get('market_regime', regime)}",
                })
        cnt = int(profile.get('sample_count', 0) or 0)
        wr = float(profile.get('win_rate', 0) or 0)
        avg = float(profile.get('avg_pnl', 0) or 0)
        if cnt >= 4:
            profile['ready'] = True
            if cnt >= 10 and (wr < 45 or avg < -0.25):
                profile['hard_block'] = True
                profile['threshold_adjust'] = 999
                profile['note'] = 'AI回測顯示此策略偏弱，暫停自動下單'
            elif wr >= 62 and avg > 0:
                bonus = min(6.0, 1.0 + (wr - 60) / 6.0 + min(avg, 2.5) * 0.35)
                profile['threshold_adjust'] = -round(bonus, 2)
                profile['note'] = 'AI回測優勢，允許更主動進場'
            elif wr >= 56 and avg >= 0:
                profile['threshold_adjust'] = -2.0
                profile['note'] = 'AI回測偏強，門檻小幅放寬'
            elif wr < 50 or avg < 0:
                malus = min(8.0, 1.5 + max(0, 52 - wr) / 4.0 + abs(min(avg, 0)) * 0.5)
                profile['threshold_adjust'] = round(malus, 2)
                profile['note'] = 'AI回測偏弱，提高門檻保守處理'
            else:
                profile['threshold_adjust'] = 0.0
                profile['note'] = 'AI回測中性，沿用主策略門檻'
    except Exception as e:
...
def ai_decide_trade(sig, eff_threshold, mkt_ok, side_ok, same_dir_cnt, pos_syms, already_closing):
    ...
    ai_threshold = max(48.0, float(eff_threshold) + float(profile.get('threshold_adjust', 0) or 0))
    min_entry_quality = 2.0 if score >= max(ai_threshold + 4, 54) else 2.5
    base_ok = (
        score >= ai_threshold
        and eq >= min_entry_quality
        and rr >= MIN_RR_HARD_FLOOR
        and side_ok
    ...
    ai_ok = not bool(profile.get('hard_block'))
    reasons = []
    if profile.get('ready'):
        reasons.append('AI接管決策')
        reasons.append('AI樣本{}'.format(profile.get('sample_count', 0)))
        reasons.append('AI勝率{:.1f}%'.format(float(profile.get('win_rate', 0) or 0)))
        reasons.append('AI均值{:+.2f}%'.format(float(profile.get('avg_pnl', 0) or 0)))
        reasons.append('AI門檻{:.1f}'.format(ai_threshold))
        if profile.get('note'):
            reasons.append(str(profile.get('note')))
    else:
        reasons.append('AI樣本不足，暫沿用主策略')
    if not ai_ok:
    ...
    return {
        ...
        'ai_ok': bool(ai_ok),
        'side': side,
        'min_entry_quality': round(min_entry_quality, 2),
    }
...
def learn_from_closed_trade(trade_id):
    ...
    try:
        ...
        with LEARN_LOCK:
            ...
            if sym not in ss: ss[sym]={"win":0,"loss":0,"count":0,"total_pnl":0.0,"total_margin_pct":0.0}
            ss[sym]["count"]+=1; ss[sym]["total_pnl"]+=pnl_pct; ss[sym]["total_margin_pct"]+=float(trade.get("margin_pct", RISK_PCT))
...
def _enhanced_auto_learn():
    with LEARN_LOCK:
        closed = [t for t in LEARN_DB.get('trades', []) if t.get('result') in ('win', 'loss')]
        total = len(closed)
    if total < 30:
        return
    db = AI_DB
    ...
    for t in closed[-180:]:
        ...
        rec = combo_stats.setdefault(key, {'count': 0, 'win': 0, 'pnl_sum': 0.0})
        rec['count'] += 1
        if t.get('result') == 'win': rec['win'] += 1
        rec['pnl_sum'] += float(t.get('pnl_pct', 0) or 0)
        regime = (t.get('breakdown', {}) or {}).get('Regime', 'neutral')
        rs = regime_stats.setdefault(regime, {'count': 0, 'win': 0, 'pnl_sum': 0.0})
        rs['count'] += 1
        if t.get('result') == 'win': rs['win'] += 1
        rs['pnl_sum'] += float(t.get('pnl_pct', 0) or 0)
        sym = t.get('symbol', 'NA')
        sk = f'{sym}|{regime}'
        sr = symbol_regime_stats.setdefault(sk, {'count': 0, 'win': 0, 'pnl_sum': 0.0})
        sr['count'] += 1
        if t.get('result') == 'win': sr['win'] += 1
        sr['pnl_sum'] += float(t.get('pnl_pct', 0) or 0)
    board = []
    for key, rec in combo_stats.items():
        if rec['count'] < 3: continue
        wr = rec['win'] / max(rec['count'], 1)
        avg = rec['pnl_sum'] / max(rec['count'], 1)
        score = wr * 70 + avg * 3 + min(rec['count'], 20)
        board.append({'strategy': key,'count': rec['count'],'win_rate': round(wr * 100, 1),'avg_pnl': round(avg, 2),'score': round(score, 2)})
    board.sort(key=lambda x: (x['score'], x['win_rate'], x['avg_pnl']), reverse=True)
    db['strategy_scoreboard'] = board[:12]
    recent = closed[-60:]
    if recent:
        avg = sum(float(t.get('pnl_pct', 0) or 0) for t in recent) / len(recent)
        wr = sum(1 for t in recent if t.get('result') == 'win') / len(recent)
        for regime, p in db.get('param_sets', {}).items():
            if wr >= 0.58 and avg > 0:
                p['tp_mult'] = round(min(float(p.get('tp_mult', 3.0)) * 1.03, 5.5), 2)
                p['trail_pct'] = round(max(float(p.get('trail_pct', 0.03)) * 0.98, 0.015), 4)
            elif wr < 0.45:
                p['sl_mult'] = round(min(float(p.get('sl_mult', 2.0)) * 1.04, 3.2), 2)
                p['tp_mult'] = round(max(float(p.get('tp_mult', 3.0)) * 0.97, 1.8), 2)
...
def run_multi_market_backtest(symbols=None):
    ...
    for idx, sym in enumerate(symbols, start=1):
        try:
            ...
            result_row = {
                ...
                'profit_factor': round(float(bt.get('profit_factor', 0) or 0), 3) if bt.get('profit_factor') is not None else None,
                'trades': int(bt.get('trades', bt.get('total_trades', 0)) or 0),
            ...
            sample_conf = min(max(result_row['trades'] / 12.0, 0), 1)
            weighted_wr = 50 + (result_row['win_rate'] - 50) * sample_conf
            weighted_ret = result_row['return_pct'] * sample_conf
            scoreboard.append({
                'strategy': '{}|{}'.format(sym, regime_info.get('regime', 'neutral')),
                'count': result_row['trades'],
                'win_rate': result_row['win_rate'],
                'avg_pnl': result_row['return_pct'],
                'score': round(weighted_wr + weighted_ret * 2 + min(result_row['trades'], 24) * 0.8, 2),
    ...
    results.sort(key=lambda x: (x.get('return_pct', 0), x.get('win_rate', 0), x.get('trades', 0)), reverse=True)
    scoreboard.sort(key=lambda x: (x.get('score', 0), x.get('win_rate', 0), x.get('count', 0)), reverse=True)
    with AI_LOCK:
        AUTO_BACKTEST_STATE['running'] = False
...
# =====================================================
# V21 DB AI：SQLite 學習資料庫 / 最近學習事件 / 回測即時學習
# =====================================================
import sqlite3
from contextlib import closing

AI_SQLITE_PATH = os.getenv('AI_SQLITE_PATH', '/app/data/ai_learning.sqlite3')

def _db_conn():
    os.makedirs(os.path.dirname(AI_SQLITE_PATH), exist_ok=True)
    conn = sqlite3.connect(AI_SQLITE_PATH, timeout=30)
...
def init_ai_sqlite():
    try:
        with closing(_db_conn()) as conn:
            cur = conn.cursor()
            cur.execute("""CREATE TABLE IF NOT EXISTS learning_events(
            ...
            cur.execute("""CREATE TABLE IF NOT EXISTS strategy_scores(
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ts TEXT NOT NULL,
            ...
            cur.execute("""CREATE TABLE IF NOT EXISTS param_history(
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ts TEXT NOT NULL,
                regime TEXT NOT NULL,
                sl_mult REAL,
                tp_mult REAL,
                breakeven_atr REAL,
                trail_trigger_atr REAL,
                trail_pct REAL,
                payload_json TEXT
            )""")
            cur.execute("""CREATE TABLE IF NOT EXISTS market_meta(
                key TEXT PRIMARY KEY,
                value TEXT
            )""")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_learning_events_ts ON learning_events(ts DESC)")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_strategy_scores_ts ON strategy_scores(ts DESC)")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_backtest_runs_ts ON backtest_runs(run_ts DESC)")
            conn.commit()
    except Exception as e:
        print('SQLite 初始化失敗:', e)

def _record_learning_event(source, summary, payload=None):
    ts = tw_now_str('%Y-%m-%d %H:%M:%S')
    try:
        with closing(_db_conn()) as conn:
            conn.execute(
                "INSERT INTO learning_events(ts, source, summary, payload_json) VALUES (?,?,?,?)",
                (ts, str(source), str(summary), json.dumps(payload or {}, ensure_ascii=False))
            )
            conn.commit()
        with AI_LOCK:
            AI_PANEL['last_learning'] = f'{ts}｜{summary}'
    except Exception as e:
        print('learning_event 寫入失敗:', e)

def _latest_learning_text():
    try:
        with closing(_db_conn()) as conn:
            row = conn.execute(
                "SELECT ts, summary FROM learning_events ORDER BY id DESC LIMIT 1"
            ).fetchone()
            if row:
                return f"{row['ts']}｜{row['summary']}"
    except Exception as e:
        print('讀取最近學習失敗:', e)
    return '--'

def _sqlite_upsert_meta(key, value):
    try:
        with closing(_db_conn()) as conn:
            conn.execute(
        ...
        print('market_meta 寫入失敗:', e)

def _sqlite_save_ai_db_snapshot(db):
    try:
        ts = tw_now_str('%Y-%m-%d %H:%M:%S')
        scoreboard = (db or {}).get('strategy_scoreboard', [])[:20]
        params = (db or {}).get('param_sets', {})
        with closing(_db_conn()) as conn:
            cur = conn.cursor()
            if scoreboard:
                cur.execute("DELETE FROM strategy_scores WHERE id IN (SELECT id FROM strategy_scores ORDER BY id DESC LIMIT 500 OFFSET 500)")
                for item in scoreboard:
                    symbol = str(item.get('symbol') or (item.get('strategy', 'NA').split('|')[-1] if item.get('strategy') else 'NA'))
                    regime = str(item.get('regime') or (item.get('strategy', 'neutral').split('|')[0] if item.get('strategy') else 'neutral'))
                    cur.execute(
                        """INSERT INTO strategy_scores(ts,symbol,regime,trades,win_rate,avg_pnl,score,confidence,timeframes,payload_json)
                           VALUES (?,?,?,?,?,?,?,?,?,?)""",
                        (
                            ts,
                            symbol,
                            regime,
                            int(item.get('trades') or item.get('count') or 0),
                            float(item.get('win_rate') or 0),
                            float(item.get('avg_pnl') or 0),
                            float(item.get('score') or 0),
                            float(item.get('confidence') or 0),
                            '/'.join(item.get('timeframes') or AI_MARKET_TIMEFRAMES),
            ...
            if params:
                for regime, p in params.items():
                    cur.execute(
                        """INSERT INTO param_history(ts,regime,sl_mult,tp_mult,breakeven_atr,trail_trigger_atr,trail_pct,payload_json)
                           VALUES (?,?,?,?,?,?,?,?)""",
                        (
                            ts, str(regime),
                            float(p.get('sl_mult', 0) or 0),
                            float(p.get('tp_mult', 0) or 0),
                            float(p.get('breakeven_atr', 0) or 0),
                            float(p.get('trail_trigger_atr', 0) or 0),
                            float(p.get('trail_pct', 0) or 0),
                            json.dumps(p, ensure_ascii=False)
                        )
                    )
            conn.commit()
        _sqlite_upsert_meta('market_history_meta', (db or {}).get('market_history_meta', {}))
        _sqlite_upsert_meta('last_learning', (db or {}).get('last_learning', '--'))
    except Exception as e:
        print('SQLite AI 快照寫入失敗:', e)

def _sqlite_store_backtest_results(results, timeframe='multi'):
    if not results:
        return
    ts = tw_now_str('%Y-%m-%d %H:%M:%S')
    try:
        with closing(_db_conn()) as conn:
            ...
            for item in results[:30]:
                cur.execute(
                    """INSERT INTO backtest_runs(run_ts,symbol,timeframe,trades,win_rate,avg_pnl,score,regime,confidence,payload_json)
                       VALUES (?,?,?,?,?,?,?,?,?,?)""",
                    (
                        ts,
                        str(item.get('symbol') or 'NA'),
                        str(item.get('timeframe') or timeframe),
                        int(item.get('trades') or item.get('count') or 0),
                        float(item.get('win_rate') or 0),
                        float(item.get('avg_pnl') or 0),
                        float(item.get('score') or 0),
                        str(item.get('market_regime') or item.get('regime') or 'neutral'),
                        float(item.get('confidence') or 0),
                        json.dumps(item, ensure_ascii=False)
                    )
                )
        ...
        print('backtest_runs 寫入失敗:', e)

def _db_learn_from_backtests(results):
    """根據前70市場回測，即時更新參數與最近學習，不必等 30 筆實單。"""
    try:
        valid = [r for r in (results or []) if float(r.get('win_rate', 0) or 0) > 0 and int(r.get('trades') or 0) >= 5]
        if not valid:
            return
        top = sorted(valid, key=lambda x: (float(x.get('score', 0) or 0), float(x.get('win_rate', 0) or 0), float(x.get('avg_pnl', 0) or 0)), reverse=True)[:12]
        db = AI_DB
        board = []
        regime_buckets = {}
        for r in top:
            regime = str(r.get('market_regime') or r.get('regime') or 'neutral')
            regime_buckets.setdefault(regime, []).append(r)
            conf = min(95, max(20, int((float(r.get('win_rate', 0) or 0) * 0.6) + min(int(r.get('trades') or 0), 30))))
            board.append({
                'symbol': r.get('symbol'),
                'regime': regime,
                'trades': int(r.get('trades') or 0),
                'win_rate': round(float(r.get('win_rate') or 0), 2),
                'avg_pnl': round(float(r.get('avg_pnl') or 0), 4),
                'score': round(float(r.get('score') or 0), 2),
                'confidence': conf,
                'timeframes': AI_MARKET_TIMEFRAMES,
                'updated_at': tw_now_str('%Y-%m-%d %H:%M:%S')
            })
        board.sort(key=lambda x: (x['score'], x['win_rate'], x['avg_pnl'], x['trades']), reverse=True)
        ...
        for regime, rows in regime_buckets.items():
            if not rows:
                continue
            wr = sum(float(x.get('win_rate', 0) or 0) for x in rows) / len(rows)
            avg = sum(float(x.get('avg_pnl', 0) or 0) for x in rows) / len(rows)
            p = db.setdefault('param_sets', {}).setdefault(regime, db.get('param_sets', {}).get('neutral', {
                'sl_mult': 2.0, 'tp_mult': 3.3, 'breakeven_atr': 0.9, 'trail_trigger_atr': 1.5, 'trail_pct': 0.035
            }).copy())
            if wr >= 68 and avg > 0.05:
                p['tp_mult'] = round(min(float(p.get('tp_mult', 3.0)) * 1.04, 5.8), 2)
                p['trail_pct'] = round(max(float(p.get('trail_pct', 0.03)) * 0.97, 0.012), 4)
            elif wr <= 48:
                p['sl_mult'] = round(min(float(p.get('sl_mult', 2.0)) * 1.03, 3.4), 2)
                p['tp_mult'] = round(max(float(p.get('tp_mult', 3.0)) * 0.98, 1.8), 2)
                p['breakeven_atr'] = round(min(float(p.get('breakeven_atr', 0.9)) * 1.02, 1.6), 2)

        best = board[0]
        summary = f"回測學習 {best['symbol']} {best['regime']}｜樣本 {best['trades']}｜勝率 {best['win_rate']}%"
        db['last_learning'] = tw_now_str('%Y-%m-%d %H:%M:%S')
        _record_learning_event('backtest', summary, {'best': best, 'count': len(board)})
        save_ai_db(db)
        with AI_LOCK:
            AI_PANEL['best_strategies'] = board[:12]
            AI_PANEL['last_learning'] = _latest_learning_text()
            best_regime = str(best.get('regime') or 'neutral')
            AI_PANEL['params'].update(db.get('param_sets', {}).get(best_regime, db.get('param_sets', {}).get('neutral', {})))
    except Exception as e:
        print('回測即時學習失敗:', e)

# 包裝原始 save_ai_db，寫 JSON 同時寫 SQLite
_BASE_SAVE_AI_DB_V21 = save_ai_db
def save_ai_db(db):
    _BASE_SAVE_AI_DB_V21(db)
    _sqlite_save_ai_db_snapshot(db)
...
def _enhanced_auto_learn():
    ...
    for key, rec in combo_stats.items():
        ...
        wr = rec['win'] / max(rec['count'], 1)
        avg = rec['pnl_sum'] / max(rec['count'], 1)
        conf = min(95, int(wr * 60 + min(rec['count'], 35)))
        score = wr * 70 + avg * 4 + min(rec['count'], 25)
        parts = str(key).split('|')
        board.append({
            'strategy': key,
            'symbol': parts[-1] if parts else 'NA',
            'regime': parts[0] if parts else 'neutral',
            'trades': rec['count'],
    ...
    db['last_learning'] = tw_now_str('%Y-%m-%d %H:%M:%S')
    save_ai_db(db)
    summary = f"實單學習 已完成 {total} 筆閉單｜策略 {len(board)} 組"
    _record_learning_event('trade', summary, {'closed_total': total, 'board_top': board[:5]})
    with AI_LOCK:
        AI_PANEL['best_strategies'] = db.get('strategy_scoreboard', [])[:8]
        AI_PANEL['last_learning'] = _latest_learning_text()
        AI_PANEL['params'].update(db.get('param_sets', {}).get('neutral', {}))

# 包裝多市場回測：每次回測後寫入 SQLite 並做即時學習
_BASE_RUN_MULTI_MARKET_BACKTEST_V21 = run_multi_market_backtest
def run_multi_market_backtest():
    results = _BASE_RUN_MULTI_MARKET_BACKTEST_V21()
    try:
        _sqlite_store_backtest_results(results, timeframe='multi')
        _db_learn_from_backtests(results)
    except Exception as e:
        print('回測包裝學習失敗:', e)
    return results

# 加強同步：最近學習直接從 SQLite 帶到前端
_BASE_SYNC_AI_STATE_TO_DASHBOARD_V21 = sync_ai_state_to_dashboard
def sync_ai_state_to_dashboard(force_regime=False):
    _BASE_SYNC_AI_STATE_TO_DASHBOARD_V21(force_regime=force_regime)
    try:
        latest = _latest_learning_text()
        with AI_LOCK:
            if latest and latest != '--':
                AI_PANEL['last_learning'] = latest
            ai_panel = dict(AI_PANEL)
            auto_bt = dict(AUTO_BACKTEST_STATE)
        update_state(ai_panel=ai_panel, auto_backtest=auto_bt)
    except Exception as e:
        print('V21 AI 狀態同步失敗:', e)

@app.route('/api/ai_db_stats')
def api_ai_db_stats():
    try:
        with closing(_db_conn()) as conn:
            latest = conn.execute("SELECT ts, source, summary FROM learning_events ORDER BY id DESC LIMIT 5").fetchall()
            strat_count = conn.execute("SELECT COUNT(1) c FROM strategy_scores").fetchone()['c']
            bt_count = conn.execute("SELECT COUNT(1) c FROM backtest_runs").fetchone()['c']
        return jsonify({
            'ok': True,
            'latest_learning': [dict(r) for r in latest],
            'strategy_rows': strat_count,
            'backtest_rows': bt_count,
            'db_path': AI_SQLITE_PATH
        })
    except Exception as e:
...
try:
    init_ai_sqlite()
    _sqlite_save_ai_db_snapshot(AI_DB)
    latest_learning = _latest_learning_text()
    if latest_learning != '--':
        with AI_LOCK:
            AI_PANEL['last_learning'] = latest_learning
    sync_ai_state_to_dashboard(force_regime=True)
    print('✅ V21 DB AI 已啟用:', AI_SQLITE_PATH)
except Exception as e:
    print('V21 DB AI 啟動失敗:', e)


# =====================================================
# V25 穩定版：備份 / 啟動順序修正 / 防重複啟動
# =====================================================
AI_BACKUP_DIR = os.getenv('AI_BACKUP_DIR', '/app/data/backups')
_THREAD_BOOTED = False
_BACKUP_META = {'last_backup': '--', 'last_backup_files': []}

def ensure_dirs():
    try:
        os.makedirs('/app/data', exist_ok=True)
        os.makedirs(AI_BACKUP_DIR, exist_ok=True)
    except Exception as e:
        print('建立資料夾失敗:', e)
...
def _safe_copy(src, dst):
    try:
        ...
        print('備份失敗 {} -> {}: {}'.format(src, dst, e))
    return False

def create_ai_backup(tag=None):
    ensure_dirs()
    ts = tw_now_str('%Y%m%d_%H%M%S')
    tag = (tag or 'auto').replace(' ', '_')
    saved = []
    pairs = [
        (AI_SQLITE_PATH, os.path.join(AI_BACKUP_DIR, f'ai_learning_{tag}_{ts}.sqlite3')),
        (LEARN_DB_PATH, os.path.join(AI_BACKUP_DIR, f'ai_learning_{tag}_{ts}.json')),
        (STATE_BACKUP_PATH, os.path.join(AI_BACKUP_DIR, f'state_{tag}_{ts}.json')),
        (RISK_STATE_PATH, os.path.join(AI_BACKUP_DIR, f'risk_{tag}_{ts}.json')),
    ]
    ...
    # 保留最近 20 份同類型備份
    try:
        files = sorted([os.path.join(AI_BACKUP_DIR, x) for x in os.listdir(AI_BACKUP_DIR)], key=os.path.getmtime, reverse=True)
        if len(files) > 80:
            for old in files[80:]:
                try:
                    os.remove(old)
        ...
        print('清理舊備份失敗:', e)
    _BACKUP_META['last_backup'] = tw_now_str('%Y-%m-%d %H:%M:%S')
    _BACKUP_META['last_backup_files'] = saved
    return saved

def sqlite_backup_thread():
    while True:
        try:
            create_ai_backup('auto')
        except Exception as e:
            print('自動備份失敗:', e)
        time.sleep(1800)

@app.route('/api/backup_now', methods=['POST', 'GET'])
def api_backup_now():
    try:
        files = create_ai_backup('manual')
        return jsonify({'ok': True, 'backup_time': _BACKUP_META['last_backup'], 'files': files})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500

# 強化 ai_db_stats，補回備份資訊
_OLD_API_AI_DB_STATS = api_ai_db_stats
@app.route('/api/ai_db_stats_v25')
def api_ai_db_stats_v25():
    try:
        base = _OLD_API_AI_DB_STATS().get_json()
    except Exception:
        base = {'ok': False}
    base = base or {}
    base['backup_time'] = _BACKUP_META.get('last_backup', '--')
    base['backup_files'] = list(_BACKUP_META.get('last_backup_files', []))
    base['db_path'] = AI_SQLITE_PATH
    return jsonify(base)

def start_all_threads_v25():
    global _THREAD_BOOTED
    if _THREAD_BOOTED:
        print('=== V25: 執行緒已啟動，略過重複啟動 ===')
        return
    _THREAD_BOOTED = True
    ensure_dirs()
    try:
        ...
        print('V25 init_ai_sqlite 失敗:', e)
    try:
        _sqlite_save_ai_db_snapshot(AI_DB)
    except Exception as e:
        print('V25 初始 SQLite 同步失敗:', e)
    load_full_state()
    load_risk_state()
    _refresh_ai_panel_market_meta()
    sync_ai_state_to_dashboard(force_regime=True)
    threads = [
        ...
        (sqlite_backup_thread,'sqlite_backup'),
    ]
    for fn, name in threads:
        t = threading.Thread(target=watchdog, args=(fn, name), daemon=True, name=name)
        t.start()
    app.view_functions['api_state'] = api_state_enhanced
    update_state(ai_panel=dict(AI_PANEL), auto_backtest=dict(AUTO_BACKTEST_STATE))
    print('=== V25 穩定版執行緒已啟動（AI DB + SQLite + 備份）===')

def post_fork(server, worker):
    start_all_threads_v25()
    print("=== [worker {}] 啟動完成 (v25) ===".format(worker.pid))
