...
def build_auto_order_reason(sig, eff_threshold, mkt_ok, side_ok, same_dir_cnt, pos_syms, already_closing):
    reasons = []
...
"tp_mult": tp_m,
            ...
            if pos_cnt < MAX_OPEN_POSITIONS:
                ...
                for best in top7_for_order:
                    ...
                    min_entry_quality = 2 if abs(best['score']) >= max(eff_threshold + 4, 54) else 2.5
                    allow_now = (abs(best['score']) >= eff_threshold
                            and entry_quality >= min_entry_quality
                            and rr_ratio >= MIN_RR_HARD_FLOOR
                            and side_ok
                            ...
                            and best['allowed']
                            and mkt_ok
                            and same_dir_cnt_now < MAX_SAME_DIRECTION
                            and can_reenter_symbol(best['symbol']))
                    reasons = build_auto_order_reason(best, eff_threshold, mkt_ok, side_ok, same_dir_cnt_now, pos_syms, already_closing)
                    AUTO_ORDER_AUDIT[best['symbol']] = {
                        'will_order': bool(allow_now),
                        'reasons': reasons or ['符合條件，自動下單'],
                        'threshold': round(eff_threshold, 2),
...
# =====================================================
# V8-lite 補強：前70成交量宇宙 / 多週期歷史資料 / 持續更新資料庫
# 只覆蓋 AI 分析與 AI 回測相關，不碰原本交易主流程。
# =====================================================

def _normalize_symbol_candidates():
    """抓 Bitget USDT 永續合約市場，依成交量排序用。"""
    try:
        ...
        for sym, m in markets.items():
            ...
            if not base or any(x in base.upper() for x in ['1000', '10000', 'BULL', 'BEAR', 'UP', 'DOWN']):
                continue
            out.append(sym)
        return out
    except Exception as e:
...
def get_top_volume_symbols(limit=70):
    """依成交額動態抓全市場前N名 USDT 永續。"""
    try:
        candidates = set(_normalize_symbol_candidates())
        tickers = exchange.fetch_tickers(list(candidates)) if candidates else exchange.fetch_tickers()
        ranked = []
        for sym, t in (tickers or {}).items():
            if sym not in candidates:
                continue
            last = float((t or {}).get('last') or 0)
            qv = float((t or {}).get('quoteVolume') or 0)
            bv = float((t or {}).get('baseVolume') or 0)
            turnover = qv if qv > 0 else bv * max(last, 0)
            if turnover <= 0:
                continue
            ranked.append((sym, turnover))
        ranked.sort(key=lambda x: x[1], reverse=True)
        top = [s for s, _ in ranked[:max(10, int(limit or 70))]]
        return top or ['BTC/USDT:USDT','ETH/USDT:USDT','SOL/USDT:USDT','XRP/USDT:USDT','DOGE/USDT:USDT']
    except Exception as e:
        print('抓前70成交量失敗:', e)
        return ['BTC/USDT:USDT','ETH/USDT:USDT','SOL/USDT:USDT','XRP/USDT:USDT','DOGE/USDT:USDT']


def _fetch_symbol_history_pack(symbol, tf_limits=None):
    tf_limits = tf_limits or {'5m': 600, '15m': 1000, '1h': 1000, '4h': 600, '1d': 365}
    pack = {}
    for tf, lim in tf_limits.items():
        try:
            data = exchange.fetch_ohlcv(symbol, tf, limit=lim)
            df = pd.DataFrame(data, columns=['t','o','h','l','c','v']) if data else pd.DataFrame(columns=['t','o','h','l','c','v'])
            if not df.empty:
                pack[tf] = df
        except Exception as e:
            print('抓歷史失敗 {} {}: {}'.format(symbol, tf, str(e)[:80]))
    return pack


def _history_snapshot_from_pack(symbol, pack, regime_info=None):
    out = {
        'symbol': symbol,
        'updated_at': tw_now_str('%Y-%m-%d %H:%M:%S'),
        'timeframes': {},
        'regime': (regime_info or {}).get('regime', 'neutral'),
        'regime_conf': (regime_info or {}).get('confidence', 0),
        'regime_dir': (regime_info or {}).get('direction', '中性'),
    }
    try:
        for tf, df in pack.items():
            ...
            v = df['v'].astype(float)
            last = float(c.iloc[-1])
            chg = float((c.iloc[-1] - c.iloc[0]) / max(c.iloc[0], 1e-9) * 100) if len(c) > 1 else 0.0
            atr = safe_last(ta.atr(h, l, c, length=14), last * 0.01)
            adx_df = ta.adx(h, l, c, length=14)
            adx = safe_last(adx_df.iloc[:, 0], 0) if adx_df is not None and not adx_df.empty else 0
            vol_ratio = float(v.tail(20).mean() / max(v.tail(120).mean() if len(v) >= 120 else v.mean(), 1e-9)) if len(v) >= 20 else 1.0
            out['timeframes'][tf] = {
                'bars': int(len(df)),
                'last': round(last, 6),
                'chg_pct': round(chg, 3),
                'atr_pct': round(float(atr) / max(last, 1e-9) * 100, 3),
...
def _persist_market_snapshot(symbol, snapshot):
    try:
        db = AI_DB
        mdb = db.setdefault('market_dataset', {})
        mdb[symbol] = snapshot
        # 只保留最近120個symbol快照
        if len(mdb) > 120:
            keys = sorted(mdb.keys())
            for k in keys[:-120]:
                mdb.pop(k, None)
        db['dataset_last_update'] = tw_now_str('%Y-%m-%d %H:%M:%S')
        save_ai_db(db)
    except Exception as e:
        print('寫入市場資料庫失敗:', e)


def _build_ai_board_from_results(results):
    board = []
    for r in results[:20]:
        tf_text = ','.join((r.get('history_timeframes') or []))
        board.append({
            'strategy': '{}|{}|{}'.format(r.get('symbol', 'NA'), r.get('market_regime', 'neutral'), r.get('market_dir', '中性')),
            'count': r.get('bars_15m', 0),
            'win_rate': round(float(r.get('win_rate', 0) or 0), 1),
            'avg_pnl': round(float(r.get('return_pct', 0) or 0), 2),
            'score': round(float(r.get('score', 0) or 0), 2),
            'note': 'TF:{}｜樣本:{}｜資料{}'.format(tf_text or '-', r.get('bars_15m', 0), r.get('dataset_updated_at', '--'))
        })
    return board
...
def run_multi_market_backtest(symbols=None):
    if symbols is None:
        symbols = get_top_volume_symbols(70)
    started_at = tw_now_str('%Y-%m-%d %H:%M:%S')
    results = []
    universe_size = len(symbols or [])
    with AI_LOCK:
        AUTO_BACKTEST_STATE['running'] = True
        AUTO_BACKTEST_STATE['last_run'] = started_at
        AUTO_BACKTEST_STATE['summary'] = 'AI 正在讀取前{}成交量市場、多週期歷史資料與滑動回測...'.format(universe_size)
        AUTO_BACKTEST_STATE['results'] = []
        AI_PANEL['regime'] = '全市場掃描中'
        AI_PANEL.setdefault('universe', {})
        AI_PANEL['universe'].update({
            'mode': 'top_volume_dynamic',
            'size': universe_size,
            'history_timeframes': ['5m','15m','1h','4h','1d'],
            'history_bars': {'5m': 600, '15m': 1000, '1h': 1000, '4h': 600, '1d': 365},
            'started_at': started_at,
            'dataset_last_update': AI_DB.get('dataset_last_update', '--'),
        })
    for idx, sym in enumerate(symbols, 1):
        try:
            hist_pack = _fetch_symbol_history_pack(sym)
            d15 = hist_pack.get('15m', pd.DataFrame())
            d1h = hist_pack.get('1h', pd.DataFrame())
            regime = classify_market_regime(d15, d1h) if not d15.empty else {'regime': 'neutral', 'direction': '中性', 'confidence': 0.3, 'note': '15m資料不足'}
            snapshot = _history_snapshot_from_pack(sym, hist_pack, regime)
            _persist_market_snapshot(sym, snapshot)
            bt = _BASE_RUN_SIMPLE_BACKTEST(symbol=sym, timeframe='15m', limit=900)
            if bt.get('ok'):
                win_rate = float(bt.get('win_rate', 0) or 0)
                ret = float(bt.get('return_pct', 0) or 0)
                pf = float(bt.get('profit_factor', 0) or 0)
                score = round(ret + win_rate * 0.55 + min(pf, 4.0) * 8 + float(regime.get('confidence', 0) or 0) * 8, 2)
                results.append({
                    'symbol': sym,
                    'win_rate': win_rate,
                    'return_pct': ret,
                    ...
                    'market_dir': regime.get('direction', '中性'),
                    'regime_conf': regime.get('confidence', 0),
                    'bars_15m': int(len(d15)),
                    'history_timeframes': list(snapshot.get('timeframes', {}).keys()),
                    'dataset_updated_at': snapshot.get('updated_at', '--'),
                    'score': score,
                })
            with AI_LOCK:
                AUTO_BACKTEST_STATE['summary'] = 'AI 回測進行中 {}/{}｜最新 {} {}'.format(idx, universe_size, sym, regime.get('regime', 'neutral'))
        except Exception as e:
            print('multi backtest失敗 {}: {}'.format(sym, e))
    results.sort(key=lambda x: (x.get('score', 0), x.get('return_pct', 0), x.get('win_rate', 0)), reverse=True)
    finished_at = tw_now_str('%Y-%m-%d %H:%M:%S')
    top_results = results[:15]
    with AI_LOCK:
        AUTO_BACKTEST_STATE['running'] = False
        AUTO_BACKTEST_STATE['last_run'] = finished_at
        AUTO_BACKTEST_STATE['summary'] = '完成前{}成交量市場掃描，成功回測 {} 個幣種，資料庫已更新'.format(universe_size, len(results))
        AUTO_BACKTEST_STATE['results'] = top_results
        AI_PANEL['last_backtest'] = finished_at
        AI_PANEL['best_strategies'] = _build_ai_board_from_results(top_results)
        AI_PANEL.setdefault('universe', {})
        AI_PANEL['universe'].update({
            'mode': 'top_volume_dynamic',
            'size': universe_size,
            'history_timeframes': ['5m','15m','1h','4h','1d'],
            'history_bars': {'5m': 600, '15m': 1000, '1h': 1000, '4h': 600, '1d': 365},
            'dataset_last_update': finished_at,
            'completed_symbols': len(results),
        })
    AI_DB.setdefault('backtests', []).append({'time': finished_at, 'results': top_results, 'universe_size': universe_size})
    AI_DB['backtests'] = AI_DB['backtests'][-30:]
    AI_DB['dataset_last_update'] = finished_at
    save_ai_db(AI_DB)
...
def api_state():
    ...
    try:
        ...
        payload['ai_dataset_last_update'] = AI_DB.get('dataset_last_update', '--')
        payload['ai_market_dataset_size'] = len((AI_DB.get('market_dataset', {}) or {}))
...
def run_multi_market_backtest(symbols=None):
    ...
    for sym in symbols:
        try:
            bt = run_simple_backtest(symbol=sym, timeframe='15m', limit=600)
            if bt.get('ok'):
                results.append({'symbol': sym,'win_rate': bt.get('win_rate', 0),'return_pct': bt.get('return_pct', 0),'profit_factor': bt.get('profit_factor'),'market_regime': (bt.get('market_regime') or {}).get('regime', 'neutral')})
        except Exception as e:
            print('multi backtest失敗 {}: {}'.format(sym, e))
    results.sort(key=lambda x: (x.get('return_pct', 0), x.get('win_rate', 0)), reverse=True)
    with AI_LOCK:
        AUTO_BACKTEST_STATE['running'] = False
        AUTO_BACKTEST_STATE['last_run'] = tw_now_str('%Y-%m-%d %H:%M:%S')
        AUTO_BACKTEST_STATE['summary'] = '完成 {} 個市場自動回測'.format(len(results))
        AUTO_BACKTEST_STATE['results'] = results[:10]
        AI_PANEL['last_backtest'] = AUTO_BACKTEST_STATE['last_run']
        if results:
            AI_PANEL['best_strategies'] = [{'strategy': '{}|{}'.format(r['symbol'], r['market_regime']),'count': 1,'win_rate': r['win_rate'],'avg_pnl': r['return_pct'],'score': round(r['return_pct'] + r['win_rate'], 2)} for r in results[:6]]
    AI_DB.setdefault('backtests', []).append({'time': AUTO_BACKTEST_STATE['last_run'],'results': results[:10]})
    AI_DB['backtests'] = AI_DB['backtests'][-30:]
    save_ai_db(AI_DB)
    update_state(ai_panel=dict(AI_PANEL), auto_backtest=dict(AUTO_BACKTEST_STATE))
    return results
...
def api_force_backtest():
    try:
        ...
        return jsonify({'ok': True, 'results': results, 'summary': AUTO_BACKTEST_STATE.get('summary', '')})
    except Exception as e:
