...
def _live_trade_stats(symbol=None, regime=None):
    ...
    pnls = [float(t.get("pnl_pct", 0) or 0) for t in rows]
...
def _ai_status_from_live(stats):
    ...
    if cnt < 20:
        return "warmup"
    if (pf is not None and float(pf) < 1.15) or ev <= 0 or dd > 12:
        return "reject"
    return "valid"
...
def _ai_strategy_profile(symbol, regime='neutral', setup=''):
    ...
    try:
        ...
        profile.update({
            ...
            'confidence': conf,
            'status': status,
            'ready': status == 'valid' and conf >= 0.45 and cnt >= 20,
        })
        ...
        if cnt < 20:
            ...
            th_adj = 0.0
            if pf is not None and pf >= 1.4:
                th_adj -= 2.0
            elif pf is not None and pf < 1.15:
                th_adj += 2.0
            if ev >= 0.12:
                th_adj -= 1.5
            elif ev <= 0:
...
def ai_decide_trade(sig, eff_threshold, mkt_ok, side_ok, same_dir_cnt, pos_syms, already_closing):
    ...
    warmup = _ai_warmup_mode()
    base_threshold = float(eff_threshold)
    if warmup:
        base_threshold = max(40.0, base_threshold - 6.0)
    ai_threshold = max(40.0 if warmup else 48.0, base_threshold + float(profile.get('threshold_adjust', 0) or 0))
    ...
    if mode == 'breakout':
        ...
        rr_floor = max(1.35, rr_floor - 0.1)

    min_entry_quality = 1.5 if warmup else (2.0 if score >= max(ai_threshold + 4, 54) else 2.5)
    if mode == 'breakout':
        min_entry_quality = max(min_entry_quality, 2.5)
    ...
    if warmup:
        reasons.append('暖機模式')
        reasons.append('AI只記錄實單，不吃回測')
        reasons.append('門檻放寬至 {:.1f}'.format(ai_threshold))
    if profile.get('ready'):
        reasons.append('AI接管決策')
    else:
        reasons.append('AI樣本不足，沿用主策略（回測僅供候選）')
    if profile.get('sample_count'):
    ...
    with LEARN_LOCK:
        ...
        for t in live_closed:
            ...
            if pkey:
                ...
                st["sample_count"] += 1
                st["total_pnl"] += float(t.get("pnl_pct",0) or 0)
                if t.get("result") == "win":
                    st["win"] += 1
            sym = str(t.get("symbol") or "")
            if sym:
                ss = sym_stats.setdefault(sym, {"win":0,"count":0})
                ss["count"] += 1
                if t.get("result") == "win":
                    ss["win"] += 1
        ...
        avg_pnl = round(sum(float(t.get("pnl_pct",0) or 0) for t in live_closed) / max(total_trades,1), 2) if total_trades else 0.0
        summary = {
