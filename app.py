...
def _apply_regime_to_signal(symbol, score, desc, entry, sl, tp, est_pnl, breakdown, atr, atr15, atr4h, sl_mult, tp_mult):
    ...
    if regime == 'trend':
        ...
        if rr >= 1.5:
            score_boost += 4 if score >= 0 else -4
            extra.append('趨勢盤加權')
    elif regime == 'range':
        score_boost += -3 if score >= 0 else 3
        extra.append('區間盤降追價')
    elif regime == 'news':
        if abs(score) >= 58:
            score_boost += 6 if score >= 0 else -6
            extra.append('消息暴衝盤強訊號加權')
        else:
            score_boost += -6 if score >= 0 else 6
            extra.append('消息盤弱訊號降權')
    if entry and atr15:
        ...
        breakdown['RegimeConf'] = regime_info.get('confidence', 0)
        breakdown['RR'] = round(rr_ratio, 2)
    score = round(score + score_boost, 1)
    desc = (desc + '|' if desc else '') + '市場:' + regime + ('|' + '|'.join(extra) if extra else '')
    with AI_LOCK:
        AI_PANEL['symbol_regimes'][symbol] = regime_info
