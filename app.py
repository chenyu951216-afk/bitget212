import os, sys, ccxt, threading, time, requests, gc, json, math, sqlite3, shutil
import numpy as np
...
# =====================================================
# V23 整合版：不卡 UI + SQLite 自動備份 + AI DB 統計
# =====================================================
SQLITE_AI_PATH = os.getenv('SQLITE_AI_PATH', '/app/data/ai_learning.sqlite3')
SQLITE_BACKUP_DIR = os.getenv('SQLITE_BACKUP_DIR', '/app/data/sqlite_backups')
SQLITE_BACKUP_INTERVAL_SEC = int(os.getenv('SQLITE_BACKUP_INTERVAL_SEC', '1800'))
SQLITE_BACKUP_KEEP = int(os.getenv('SQLITE_BACKUP_KEEP', '48'))
_START_THREADS_DONE = False
_START_THREADS_LOCK = threading.Lock()
_LAST_SQLITE_BACKUP = {'time': '--', 'files': []}
...
def init_ai_sqlite():
    try:
        with _db_connect() as conn:
            ...
            def _ensure_column(table_name, col_name, col_def):
                cols = {r[1] for r in conn.execute(f"PRAGMA table_info({table_name})").fetchall()}
                if col_name not in cols:
                    conn.execute(f"ALTER TABLE {table_name} ADD COLUMN {col_name} {col_def}")
            for c, d in [
                ('snapshot_time', 'TEXT'), ('symbol', 'TEXT'), ('strategy', 'TEXT'), ('regime', 'TEXT'),
                ('sample_count', 'INTEGER'), ('win_rate', 'REAL'), ('avg_pnl', 'REAL'), ('ev', 'REAL'),
                ('profit_factor', 'REAL'), ('max_drawdown', 'REAL'), ('score', 'REAL'), ('confidence', 'REAL'),
                ('timeframes', 'TEXT'), ('updated_at', 'TEXT')
            ]:
                _ensure_column('strategy_scores', c, d)
            for c, d in [
                ('run_time', 'TEXT'), ('scanned_markets', 'INTEGER'), ('target_count', 'INTEGER'),
                ('duration_sec', 'REAL'), ('summary', 'TEXT'), ('result_count', 'INTEGER'), ('error_count', 'INTEGER')
            ]:
                _ensure_column('backtest_runs', c, d)
            for c, d in [
                ('event_time', 'TEXT'), ('source', 'TEXT'), ('symbol', 'TEXT'), ('regime', 'TEXT'),
                ('sample_count', 'INTEGER'), ('win_rate', 'REAL'), ('avg_pnl', 'REAL'), ('ev', 'REAL'), ('note', 'TEXT')
            ]:
                _ensure_column('learning_events', c, d)
            conn.commit()
    except Exception as e:
        print('初始化 SQLite 失敗:', e)
...
def sync_ai_db_to_sqlite(db=None):
    try:
        init_ai_sqlite()
        db = db or AI_DB
        now_ts = tw_now_str('%Y-%m-%d %H:%M:%S')
        scoreboard = list((db or {}).get('strategy_scoreboard', []) or [])[:50]
        backtests = list((db or {}).get('backtests', []) or [])[-10:]
        last_learning = str((db or {}).get('last_learning', '--') or '--')
        note = ''
        if last_learning and last_learning != '--':
            note = str((AI_PANEL.get('last_learning') if 'AI_PANEL' in globals() else '') or '')
        with _db_connect() as conn:
            conn.execute('DELETE FROM strategy_scores WHERE snapshot_time < datetime("now","-14 day")')
            conn.execute('DELETE FROM backup_log WHERE backup_time < datetime("now","-30 day")')
            if scoreboard:
                conn.execute('DELETE FROM strategy_scores WHERE snapshot_time = ?', (now_ts,))
                for s in scoreboard:
                    symbol = str(s.get('symbol') or s.get('market') or s.get('pair') or '')
                    strategy = str(s.get('strategy') or (f"{symbol}｜{s.get('regime','neutral')}" if symbol else '--'))
                    regime = str(s.get('regime') or s.get('market_regime') or 'neutral')
                    conn.execute("""INSERT INTO strategy_scores (
                        snapshot_time, symbol, strategy, regime, sample_count, win_rate, avg_pnl, ev, profit_factor, max_drawdown, score, confidence, timeframes, updated_at
                    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
                        now_ts,
                        symbol,
                        strategy,
                        regime,
                        _safe_int(s.get('count', s.get('trades', 0))),
                        _safe_float(s.get('win_rate', 0.0)),
                        _safe_float(s.get('avg_pnl', s.get('return_pct', 0.0))),
                        _safe_float(s.get('ev', s.get('expectancy', 0.0))),
                        _safe_float(s.get('profit_factor', 0.0)),
                        _safe_float(s.get('max_drawdown', s.get('drawdown', 0.0))),
                        _safe_float(s.get('score', 0.0)),
                        _safe_float(_normalize_confidence(s.get('confidence', 0.0))),
                        str(s.get('timeframes') or s.get('timeframe') or ''),
                        str(s.get('updated_at') or now_ts),
                    ))
            for bt in backtests:
                conn.execute("""INSERT INTO backtest_runs (
                    run_time, scanned_markets, target_count, duration_sec, summary, result_count, error_count
                ) VALUES (?,?,?,?,?,?,?)""", (
                    str(bt.get('time') or now_ts),
                    _safe_int(bt.get('scanned_markets', 0)),
                    _safe_int(bt.get('target_count', 0)),
                    _safe_float(bt.get('duration_sec', 0.0)),
                    str(AUTO_BACKTEST_STATE.get('summary', '')),
                    len(bt.get('results', []) or []),
                    len(bt.get('errors', []) or []),
                ))
            if last_learning and last_learning != '--':
                top = scoreboard[0] if scoreboard else {}
                conn.execute("""INSERT INTO learning_events (
                    event_time, source, symbol, regime, sample_count, win_rate, avg_pnl, ev, note
                ) VALUES (?,?,?,?,?,?,?,?,?)""", (
                    last_learning,
                    'backtest',
                    str(top.get('symbol') or ''),
                    str(top.get('regime') or top.get('market_regime') or 'neutral'),
                    _safe_int(top.get('count', top.get('trades', 0))),
                    _safe_float(top.get('win_rate', 0.0)),
                    _safe_float(top.get('avg_pnl', top.get('return_pct', 0.0))),
                    _safe_float(top.get('ev', top.get('expectancy', 0.0))),
                    note[:500],
                ))
            conn.commit()
    except Exception as e:
        print('同步 SQLite 失敗:', e)


_BASE_SAVE_AI_DB_V23 = save_ai_db

def save_ai_db(db):
    _BASE_SAVE_AI_DB_V23(db)
    try:
        sync_ai_db_to_sqlite(db)
    except Exception as e:
        print('save_ai_db 同步 SQLite 失敗:', e)


def backup_ai_storage():
    files = []
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    try:
        os.makedirs(SQLITE_BACKUP_DIR, exist_ok=True)
        if os.path.exists(SQLITE_AI_PATH):
            dst = os.path.join(SQLITE_BACKUP_DIR, f'ai_learning_{ts}.sqlite3')
            with _db_connect(SQLITE_AI_PATH) as src, sqlite3.connect(dst) as out:
                src.backup(out)
            files.append(dst)
        for extra in [AI_DB_PATH, STATE_BACKUP_PATH, RISK_STATE_PATH]:
            if os.path.exists(extra):
                name = os.path.basename(extra)
                dst = os.path.join(SQLITE_BACKUP_DIR, f'{ts}_{name}')
                shutil.copy2(extra, dst)
                files.append(dst)
        backups = sorted([p for p in os.listdir(SQLITE_BACKUP_DIR) if p.endswith('.sqlite3') or p.endswith('.json')], reverse=True)
        for old in backups[SQLITE_BACKUP_KEEP:]:
            try:
                os.remove(os.path.join(SQLITE_BACKUP_DIR, old))
            except Exception:
                pass
        _LAST_SQLITE_BACKUP['time'] = tw_now_str('%Y-%m-%d %H:%M:%S')
        _LAST_SQLITE_BACKUP['files'] = files[-6:]
        try:
            with _db_connect() as conn:
                for f in files:
                    conn.execute('INSERT INTO backup_log (backup_time, file_path, status, note) VALUES (?,?,?,?)', (_LAST_SQLITE_BACKUP['time'], f, 'ok', 'auto backup'))
                conn.commit()
        except Exception as e:
            print('寫入 backup_log 失敗:', e)
        return {'ok': True, 'time': _LAST_SQLITE_BACKUP['time'], 'files': files}
    except Exception as e:
        err = str(e)
        _LAST_SQLITE_BACKUP['time'] = tw_now_str('%Y-%m-%d %H:%M:%S')
        _LAST_SQLITE_BACKUP['files'] = []
        try:
            with _db_connect() as conn:
                conn.execute('INSERT INTO backup_log (backup_time, file_path, status, note) VALUES (?,?,?,?)', (_LAST_SQLITE_BACKUP['time'], '', 'fail', err[:300]))
                conn.commit()
        except Exception:
            pass
...
def sqlite_backup_thread():
    while True:
        try:
            ...
            print('SQLite 自動備份失敗:', e)
        time.sleep(max(300, SQLITE_BACKUP_INTERVAL_SEC))


def _ai_db_stats_dict():
    try:
        init_ai_sqlite()
        with _db_connect() as conn:
            out = {
            ...
            try:
                out['strategy_rows'] = conn.execute('SELECT COUNT(*) FROM strategy_scores').fetchone()[0]
            except Exception:
                out['strategy_rows'] = 0
            try:
                out['learning_rows'] = conn.execute('SELECT COUNT(*) FROM learning_events').fetchone()[0]
            except Exception:
                out['learning_rows'] = 0
            try:
                out['backtest_rows'] = conn.execute('SELECT COUNT(*) FROM backtest_runs').fetchone()[0]
            except Exception:
                out['backtest_rows'] = 0
            try:
                cols = {r[1] for r in conn.execute('PRAGMA table_info(learning_events)').fetchall()}
                if 'event_time' in cols:
                    row = conn.execute('SELECT event_time, source, symbol, regime, sample_count, win_rate, avg_pnl, ev, note FROM learning_events ORDER BY id DESC LIMIT 1').fetchone()
                    if row:
                        keys = ['event_time','source','symbol','regime','sample_count','win_rate','avg_pnl','ev','note']
                        out['latest_learning_event'] = dict(zip(keys, row))
            except Exception:
                pass
            try:
                row = conn.execute("SELECT backup_time FROM backup_log WHERE status='ok' ORDER BY id DESC LIMIT 1").fetchone()
                if row and row[0]:
                    out['last_backup_time'] = row[0]
            except Exception:
                pass
            return out
    except Exception as e:
        return {'strategy_rows': 0, 'learning_rows': 0, 'backtest_rows': 0, 'latest_learning_event': {}, 'last_backup_time': '--', 'error': str(e)}

@app.route('/api/ai_db_stats', methods=['GET'])
def api_ai_db_stats():
    return jsonify({'ok': True, **_ai_db_stats_dict()})


@app.route('/api/backup_now', methods=['POST'])
def api_backup_now():
    result = backup_ai_storage()
    return jsonify(result)


def api_state_v23_enhanced():
    try:
        resp = _BASE_API_STATE()
        try:
            payload = resp.get_json() or {}
        ...
        with AI_LOCK:
            payload['ai_panel'] = dict(AI_PANEL)
            payload['auto_backtest'] = dict(AUTO_BACKTEST_STATE)
        payload['ai_db_stats'] = _ai_db_stats_dict()
        return jsonify(payload)
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e), 'ai_panel': dict(AI_PANEL), 'auto_backtest': dict(AUTO_BACKTEST_STATE), 'ai_db_stats': _ai_db_stats_dict()})


app.view_functions['api_state'] = api_state_v23_enhanced


def start_all_threads():
    global _START_THREADS_DONE
    with _START_THREADS_LOCK:
        if _START_THREADS_DONE:
            print('=== 執行緒已啟動，略過重複啟動 ===')
            return
        _START_THREADS_DONE = True
    load_full_state()
    load_risk_state()
    _refresh_ai_panel_market_meta()
    sync_ai_state_to_dashboard(force_regime=True)
    init_ai_sqlite()
    sync_ai_db_to_sqlite(AI_DB)
    threads = [
        (news_thread,'news'),
        (position_thread,'position'),
    ...
    for fn, name in threads:
        t = threading.Thread(target=watchdog, args=(fn, name), daemon=True, name=name)
        t.start()
    print('=== 所有執行緒已啟動（AI DB + SQLite 備份版）===')


def post_fork(server, worker):
    start_all_threads()
    print('=== [worker {}] 啟動完成（V23） ==='.format(worker.pid))


try:
    init_ai_sqlite()
    sync_ai_db_to_sqlite(AI_DB)
except Exception as _v23_init_err:
    print('V23 初始化失敗:', _v23_init_err)
