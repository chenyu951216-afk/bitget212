...
def place_order(sig):
    ...
    try:
        ...
        rec={"symbol":sym,"side":"做多" if side=='buy' else "做空","score":sig['score'],
             ...
             "est_pnl":sig['est_pnl'],"order_usdt":round(order_usdt,2),"risk_usdt":round(est_risk_usdt,2),"leverage":lev,
            ...
            if pos_cnt < MAX_OPEN_POSITIONS:
                ...
                for best in top7_for_order:
                    ...
                    if (abs(best['score']) >= eff_threshold  # 動態門檻（含大盤調整）
                            ...
                            and best['allowed']
                            and mkt_ok
                            and same_dir_cnt_now < MAX_SAME_DIRECTION):  # 大盤方向過濾 + 同向上限
...
def run_simple_backtest(symbol="BTC/USDT:USDT", timeframe="15m", limit=800, fee_rate=0.0006):
    ...
    try:
        ...
        df['adx'] = ta.adx(df['h'], df['l'], df['c'], length=14).iloc[:, 0]
        ...
        for i in range(60, len(df)):
            ...
            if position is None:
                if ema_up and (macd_up or squeeze_long) and row['rsi'] < 68 and adx_ok and anti_chase_long:
                    entry = min(price, max(float(row['ema21']), hh - atr * PULLBACK_BUFFER_ATR)) if price >= hh * 0.999 else price
                    sl = entry - atr * 1.6
                    tp = entry + atr * (3.4 if squeeze_long else 3.0)
                    ...
                    cap_qty = (equity * margin_pct) / max(entry, 1e-9)
                    risk_qty = risk_budget / max(entry - sl, price * 0.002)
                    qty = max(min(risk_qty, cap_qty), 0)
                    position = {"side": "long", "entry": entry, "sl": sl, "tp": tp, "qty": qty, "bar": i, "margin_pct": margin_pct}
                elif ema_dn and (macd_dn or squeeze_short) and row['rsi'] > 32 and adx_ok and anti_chase_short:
                    entry = max(price, min(float(row['ema21']), ll + atr * PULLBACK_BUFFER_ATR)) if price <= ll * 1.001 else price
                    sl = entry + atr * 1.6
                    tp = entry - atr * (3.4 if squeeze_short else 3.0)
                    ...
                    risk_budget = equity * ATR_RISK_PCT
                    cap_qty = (equity * margin_pct) / max(entry, 1e-9)
                    risk_qty = risk_budget / max(sl - entry, price * 0.002)
                    qty = max(min(risk_qty, cap_qty), 0)
                    position = {"side": "short", "entry": entry, "sl": sl, "tp": tp, "qty": qty, "bar": i, "margin_pct": margin_pct}
                continue
            ...
            if exit_reason:
                ...
                trades.append({
                    ...
                    "reason": exit_reason,
                    "bars": bars_held,
                    "margin_pct": round(position.get('margin_pct', RISK_PCT) * 100, 2),
                })
        ...
        return {
            ...
            "margin_range_pct": [round(MIN_MARGIN_PCT * 100, 2), round(MAX_MARGIN_PCT * 100, 2)],
            "net_profit": round(equity - 10000.0, 2),
            "return_pct": round((equity / 10000.0 - 1) * 100, 2),
