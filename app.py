...
AI_MIN_SAMPLE_STRICT = int(os.getenv('AI_MIN_SAMPLE_STRICT', '15'))
AI_MIN_SAMPLE_DISPLAY = int(os.getenv('AI_MIN_SAMPLE_DISPLAY', '8'))
AI_MAX_ACCEPT_DD = float(os.getenv('AI_MAX_ACCEPT_DD', '18'))
AI_MIN_PROFIT_FACTOR = float(os.getenv('AI_MIN_PROFIT_FACTOR', '1.15'))
AI_EV_FLOOR = float(os.getenv('AI_EV_FLOOR', '0.0'))
...
def _ai_confidence_from_trades(trades):
    try:
        t = max(float(trades or 0), 0.0)
    except Exception:
        t = 0.0
    return round(min(t / 30.0, 1.0), 3)

def _normalize_confidence_display(v):
    x = _safe_pct(v)
    return x / 100.0 if x > 1.0 else x

def _score_backtest_row(row):
    trades = int(row.get('trades', 0) or 0)
    win_rate = _safe_pct(row.get('win_rate', 0))
    return_pct = _safe_pct(row.get('return_pct', 0))
    profit_factor = _safe_pct(row.get('profit_factor', 0))
    max_dd = _safe_pct(row.get('max_drawdown_pct', 0))
    ev_per_trade = (return_pct / trades) if trades > 0 else -999.0
    confidence = _ai_confidence_from_trades(trades)

    score = 0.0
    score += ev_per_trade * 70.0
    score += max(min(profit_factor, 4.0) - 1.0, -1.0) * 22.0
    score += (win_rate - 50.0) * 0.30
    score -= max_dd * 1.25
    score += confidence * 12.0

    if trades < AI_MIN_SAMPLE_STRICT:
        score -= (AI_MIN_SAMPLE_STRICT - trades) * 4.0
    if profit_factor and profit_factor < AI_MIN_PROFIT_FACTOR:
        score -= (AI_MIN_PROFIT_FACTOR - profit_factor) * 35.0
    if max_dd > AI_MAX_ACCEPT_DD:
        score -= (max_dd - AI_MAX_ACCEPT_DD) * 4.0
    if ev_per_trade < AI_EV_FLOOR:
        score -= abs(ev_per_trade - AI_EV_FLOOR) * 25.0
    ...
    return {
        ...
        'ev_per_trade': round(ev_per_trade, 4) if ev_per_trade != -999.0 else 0.0,
        'confidence': confidence,
        'score': round(score, 2),
        'quality_ok': bool(
            trades >= AI_MIN_SAMPLE_STRICT
            and (profit_factor == 0.0 or profit_factor >= AI_MIN_PROFIT_FACTOR)
            and max_dd <= AI_MAX_ACCEPT_DD
            and ev_per_trade >= AI_EV_FLOOR
        )
    }
    ...
    try:
        ...
        if exact_row:
            profile.update({
                ...
                'avg_pnl': _safe_pct(exact_row.get('avg_pnl', 0)),
                'ev_per_trade': _safe_pct(exact_row.get('ev_per_trade', 0)),
                'profit_factor': _safe_pct(exact_row.get('profit_factor', 0)),
                'max_drawdown_pct': _safe_pct(exact_row.get('max_drawdown_pct', 0)),
            ...
            market_row = next((r for r in market_rows if str(r.get('symbol')) == symbol and str(r.get('market_regime', regime)) == regime), None)
            if not market_row:
                market_row = next((r for r in market_rows if str(r.get('symbol')) == symbol), None)
            if market_row:
                scored = _score_backtest_row(market_row)
                profile.update({
                    'source': 'market_backtest',
                    'sample_count': scored['trades'],
                    'win_rate': scored['win_rate'],
                    'avg_pnl': scored['return_pct'],
                    'ev_per_trade': scored['ev_per_trade'],
        ...
        wr = _safe_pct(profile.get('win_rate', 0))
        ev = _safe_pct(profile.get('ev_per_trade', 0))
        pf = _safe_pct(profile.get('profit_factor', 0))
        dd = _safe_pct(profile.get('max_drawdown_pct', 0))

        if cnt >= AI_MIN_SAMPLE_DISPLAY:
            profile['ready'] = True

            if cnt >= AI_MIN_SAMPLE_STRICT and ((pf and pf < AI_MIN_PROFIT_FACTOR) or dd > AI_MAX_ACCEPT_DD or ev < AI_EV_FLOOR):
                profile['hard_block'] = True
                profile['threshold_adjust'] = 999
                profile['note'] = 'AI風險過高或期望值不足，暫停自動下單'
            elif cnt >= AI_MIN_SAMPLE_STRICT and ev > 0.05 and (pf == 0.0 or pf >= 1.25) and dd <= 12:
                profile['threshold_adjust'] = -4.0
                profile['note'] = 'AI評估正期望且回撤低，允許更主動進場'
            elif cnt >= AI_MIN_SAMPLE_STRICT and ev > 0 and (pf == 0.0 or pf >= 1.1):
                profile['threshold_adjust'] = -2.0
                profile['note'] = 'AI評估略有優勢，門檻小幅放寬'
            elif cnt >= AI_MIN_SAMPLE_STRICT and (ev < 0 or wr < 50):
                profile['threshold_adjust'] = 3.0
                profile['note'] = 'AI評估邊際偏弱，提高門檻保守處理'
            else:
                profile['threshold_adjust'] = 0.0
                profile['note'] = 'AI樣本逐步累積中，暫維持主策略'
    except Exception as e:
...
def run_multi_market_backtest(symbols=None):
    ...
    for idx, sym in enumerate(symbols, start=1):
        try:
            ...
            result_row = {
                ...
                'profit_factor': round(float(bt.get('profit_factor', 0) or 0), 3) if bt.get('profit_factor') is not None else 0.0,
                'max_drawdown_pct': round(float(bt.get('max_drawdown_pct', 0) or 0), 2),
            ...
            scored = _score_backtest_row(result_row)
            result_row['ev_per_trade'] = scored['ev_per_trade']
            result_row['score'] = scored['score']
            result_row['confidence'] = scored['confidence']
    ...
    with AI_LOCK:
        ...
        AI_DB['strategy_scoreboard'] = scoreboard[:30]
        ...
        if scoreboard:
            top = scoreboard[0]
            AI_PANEL['last_learning'] = '{}｜AI評估 {}｜EV/筆 {}｜PF {}｜DD {}%'.format(
                AUTO_BACKTEST_STATE['last_run'],
                top.get('strategy', '--'),
                top.get('ev_per_trade', 0),
                top.get('profit_factor', 0),
                top.get('max_drawdown_pct', 0),
            )
            AI_DB['last_learning'] = AI_PANEL['last_learning']
    save_ai_db(AI_DB)
    update_state(ai_panel=dict(AI_PANEL), auto_backtest=dict(AUTO_BACKTEST_STATE))
    return results
