...
# =====================================================
# 鎖分層（安全重構版）
# - 狀態類：STATE / RISK / SESSION / MARKET / LT / 動態門檻
# - 執行類：TRAILING / FVG / ORDER / 本輪下單去重
# - 學習類：LEARN_DB
# - AI 類：AI_PANEL / AUTO_BACKTEST
# 這次只重構鎖，不改 API / UI / AI / 交易邏輯。
# 使用 RLock 讓同一執行緒可重入，降低交叉拿鎖造成的死鎖風險。
# =====================================================
CORE_STATE_LOCK = threading.RLock()
EXECUTION_LOCK  = threading.RLock()
LEARNING_LOCK   = threading.RLock()
AI_PANEL_LOCK   = threading.RLock()
...
def update_dynamic_threshold(top_sigs=None):
    ...
    with _DT_LOCK:
        ...
        snap = get_state_snapshot("active_positions", "top_signals")
        pos_count = len(snap.get("active_positions") or [])
        sigs = top_sigs or (snap.get("top_signals") or [])
...
def check_risk_ok():
    ...
    try:
        ...
        if rs["today_date"] != today:
            ...
            rs["daily_start_equity"] = get_state_value("equity", 0)
        ...
        # 日虧損上限
        equity = get_state_value("equity", 0)
...
def get_risk_status():
    ...
    try:
        ...
        if rs.get("cooldown_until") and datetime.now() < rs["cooldown_until"]:
            ok = False
        equity = get_state_value("equity", 1)
...
def clear_auto_order_audit():
    with STATE_LOCK:
        AUTO_ORDER_AUDIT.clear()

def set_auto_order_audit(symbol, payload):
    with STATE_LOCK:
        AUTO_ORDER_AUDIT[symbol] = payload

def set_protection_state(symbol, payload):
    with STATE_LOCK:
        PROTECTION_STATE[symbol] = payload
...
# }}
TRAILING_STATE = {}
TRAILING_LOCK  = EXECUTION_LOCK
ORDER_LOCK     = EXECUTION_LOCK   # 防止同時下多筆單超過7個持倉
_ORDERED_THIS_SCAN = set()  # 本輪已下單的幣（防止同輪重複下單）
_ORDERED_LOCK = EXECUTION_LOCK
...
def scan_thread():
    ...
    while True:
        try:
            ...
            clear_auto_order_audit()

            sc = get_state_value("scan_count", 0)
...
"tp_mult": tp_m,
            ...
            if pos_cnt < MAX_OPEN_POSITIONS:
                ...
                for best in top7_for_order:
                    ...
                    set_auto_order_audit(best['symbol'], {
