        ...
        try:
            ...
            for symbol in syms:
                try:
                    ...
                    # 重新分析分數（看是否反轉）
                    analysis_result = analyze(symbol)
                    if isinstance(analysis_result, (list, tuple)) and len(analysis_result) >= 1:
                        sc = float(analysis_result[0] or 0)
                    else:
                        sc = 0.0
...
def api_state():
    ...
    try:
        ...
        if 'market_info' not in payload or not isinstance(payload.get('market_info'), dict):
            payload['market_info'] = dict(MARKET_STATE)
        if 'fvg_orders' not in payload:
            payload['fvg_orders'] = dict(FVG_ORDERS)
        return jsonify(payload)
    except Exception as e:
        print('AI wrapper api_state 失敗:', e)
        try:
            fallback = {}
            with STATE_LOCK:
                fallback.update(dict(STATE))
            with AI_LOCK:
                fallback['ai_panel'] = dict(AI_PANEL)
                fallback['auto_backtest'] = dict(AUTO_BACKTEST_STATE)
            fallback['market_info'] = dict(MARKET_STATE)
            fallback['fvg_orders'] = dict(FVG_ORDERS)
            return jsonify(fallback)
        except Exception:
            return resp

# 重要：覆蓋 Flask 先前已註冊的 /api/state endpoint，讓前端真的拿到 AI 欄位
try:
    app.view_functions['api_state'] = api_state
except Exception as e:
    print('覆蓋 api_state endpoint 失敗:', e)

def start_all_threads():
    load_full_state()
    load_risk_state()
    # 啟動前先把 AI 與回測初始狀態同步到前端，避免 UI 一直顯示 --
    try:
        with AI_LOCK:
            AI_PANEL['market_db_info'] = dict(AI_DB.get('market_history_meta', AI_PANEL.get('market_db_info', {})))
            AI_PANEL['best_strategies'] = list(AI_DB.get('strategy_scoreboard', []))[:12]
            AI_PANEL['last_learning'] = AI_DB.get('last_learning', AI_PANEL.get('last_learning', '--'))
            AUTO_BACKTEST_STATE['db_symbols'] = int((AI_PANEL.get('market_db_info') or {}).get('symbols', 0) or 0)
            AUTO_BACKTEST_STATE['db_last_update'] = (AI_PANEL.get('market_db_info') or {}).get('last_update', '--')
            AUTO_BACKTEST_STATE['data_timeframes'] = list((AI_PANEL.get('market_db_info') or {}).get('timeframes', AUTO_BACKTEST_STATE.get('data_timeframes', [])))
            if AUTO_BACKTEST_STATE['summary'] in ('尚未啟動', '', None):
                AUTO_BACKTEST_STATE['summary'] = 'AI引擎準備中，等待首次自動回測...'
    except Exception as e:
        print('AI 啟動預熱失敗:', e)

    update_state(ai_panel=dict(AI_PANEL), auto_backtest=dict(AUTO_BACKTEST_STATE), market_info=dict(MARKET_STATE), fvg_orders=dict(FVG_ORDERS))
