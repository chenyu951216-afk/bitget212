...
def _ai_status_from_live(stats):
    ...
    if cnt < 20:
        return "warmup"
    if cnt >= 12 and (wr < 38 or avg <= -0.18):
        return "reject"
    if (pf is not None and float(pf) < 1.05) or ev <= 0 or dd > 12:
        return "reject"
    if cnt < 30:
        return "observe"
    return "valid"
...
def _ai_warmup_mode():
    return len(get_live_trades(closed_only=True)) < 30
...
def _ai_strategy_profile(symbol, regime='neutral', setup=''):
    ...
    try:
        ...
        profile.update({
            ...
            'confidence': conf,
            'status': status,
            'ready': status == 'valid' and conf >= 0.30 and cnt >= 20 and fit_ok and not sym_block,
...
def ai_decide_trade(sig, eff_threshold, mkt_ok, side_ok, same_dir_cnt, pos_syms, already_closing):
    ...
    # v34: AI 真接管，暖機後改為 AI threshold 主導
    ...
    if mode == 'breakout':
        rr_floor = max(rr_floor, 1.90)
    elif mode == 'range':
        rr_floor = max(1.35, rr_floor - 0.05)
    else:
    ...
    if warmup:
        ...
        min_entry_quality = 3.0 if not profile.get('ready') else 2.6
        if mode == 'breakout':
            min_entry_quality = max(min_entry_quality, 3.2)
        if regime == 'range':
            min_entry_quality = max(min_entry_quality, 3.0)

    hard_block = bool(profile.get('hard_block'))
    if not fit_ok and not warmup:
        hard_block = True

    # 暖機後沒有 regime-fit / 沒有足夠樣本就不能亂開
    sample_count = int(profile.get('sample_count', 0) or 0)
    if not warmup and sample_count < 5 and score < max(ai_threshold + 6.0, 58.0):
        hard_block = True
...
def ai_block_bad(stats):
    try:
        if stats.get("count",0) >= 20:
            if stats.get("win_rate",50) < 45: return True
            if stats.get("avg_pnl",0) <= 0: return True
    except:
        pass
    return False

def ai_regime_filter(signal, regime):
    try:
        if regime == "neutral":
            if signal.get("setup_label","") in ["breakout","trend","continuation"]:
                return False
    except:
        pass
    return True

def ai_final_decision(signal, stats, regime="neutral"):
    if ai_block_bad(stats):
        return False, "block_bad_strategy"
    if not ai_regime_filter(signal, regime):
        return False, "regime_filter"
    return True, "pass"

print("✅ v37 AI fully loaded")

# =========================
# 使用方式（已幫你嵌入邏輯點）
# =========================
def AI_DECISION_WRAPPER(signal, stats, regime):
    ok, reason = ai_final_decision(signal, stats, regime)
    if not ok:
        return False, reason
    return True, "pass"
