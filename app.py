...
ORDER_THRESHOLD_DROP    = max(float(DECISION_POLICY.get('order_threshold_drop', 2) or 2), 2.0)    # 每空一輪下降 2 分
ORDER_THRESHOLD_FLOOR   = min(float(DECISION_POLICY.get('order_threshold_floor', 55) or 55), 46.0)   # AI主控版：最低可放寬
...
def apply_execution_guard(symbol, side, margin_pct):
    ...
    try:
        ...
        API_ERROR_STREAK = min(API_ERROR_STREAK + 1, 10)
        softened_gate = {'action': 'penalty', 'softened': True, 'score_penalty': 7.0, 'margin_mult': 0.38, 'reasons': ['execution guard error', 'execution guard 失敗改為降倉扣分']}
