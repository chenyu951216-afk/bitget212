...
def _ai_strategy_profile(symbol, regime='neutral', setup=''):
    ...
    try:
        ...
        with LEARN_LOCK:
            learn_srs = dict((LEARN_DB.get('symbol_regime_stats', {}) or {}).get(f'{symbol}|{regime}', {}) or {})

        exact_row = next((r for r in board if str(r.get('strategy')) == strategy_key), None)
        generic_row = next((r for r in board if str(r.get('strategy') or '').startswith(f'{symbol}|{regime}')), None)
        market_row = next((r for r in market_rows if str(r.get('symbol')) == symbol), None)

        if int(learn_srs.get('count', 0) or 0) > 0:
            cnt = int(learn_srs.get('count', 0) or 0)
            wr = (float(learn_srs.get('win', 0) or 0) / max(cnt, 1)) * 100.0
            avg = float(learn_srs.get('pnl_sum', 0) or 0) / max(cnt, 1)
            profile.update({
                'ready': cnt >= 20,
                'source': 'real_trades',
                'sample_count': cnt,
            ...
            if row:
                ...
                profile.update({
                    ...
                    'sample_count': cnt,
                    'win_rate': _normalize_wr_percent(row.get('win_rate', 0)),
                    'avg_pnl': float(row.get('avg_pnl', 0) or 0),
                    'ev_per_trade': float(row.get('ev_per_trade', row.get('avg_pnl', 0)) or 0),
                    'profit_factor': float(row.get('profit_factor', 0) or 0) if row.get('profit_factor') is not None else None,
                    'max_drawdown_pct': float(row.get('max_drawdown_pct', 0) or 0),
                    'strategy': str(row.get('strategy') or strategy_key),
                    'strategy_mode': str(row.get('strategy_mode') or 'main'),
                })
            elif market_row:
                ...
                profile.update({
                    ...
                    'ev_per_trade': float(market_row.get('ev_per_trade', 0) or 0),
                    'profit_factor': float(market_row.get('profit_factor', 0) or 0) if market_row.get('profit_factor') is not None else None,
                    'max_drawdown_pct': float(market_row.get('max_drawdown_pct', 0) or 0),
                    'strategy': f"{symbol}|{market_row.get('market_regime', regime)}",
                    'strategy_mode': str(market_row.get('strategy_mode') or 'main'),
                })

        cnt = int(profile.get('sample_count', 0) or 0)
        wr = float(profile.get('win_rate', 0) or 0)
        avg = float(profile.get('avg_pnl', 0) or 0)
        ev = float(profile.get('ev_per_trade', avg) or 0)
        pf = float(profile.get('profit_factor', 0) or 0) if profile.get('profit_factor') is not None else None
        dd = float(profile.get('max_drawdown_pct', 0) or 0) if profile.get('max_drawdown_pct') is not None else None

        # 策略切換：只切模式，不刪原功能
        if regime in ('news', 'breakout'):
            profile['strategy_mode'] = 'breakout'
        elif regime in ('range',):
            profile['strategy_mode'] = 'range'
        else:
            profile['strategy_mode'] = profile.get('strategy_mode') or 'main'

        if profile.get('source') == 'real_trades' and cnt >= 20:
            profile['ready'] = True
            hard_block = False
            th_adj = 0.0
            notes = []
            # 先管風險，再談勝率
            if cnt >= 10 and ((pf is not None and pf < 0.95) or ev < -0.02 or (dd is not None and dd > 12)):
                hard_block = True
                th_adj = 999
                notes.append('AI判定此策略風險過高，暫停自動下單')
            else:
                if pf is not None and pf >= 1.25: th_adj -= 1.5
                elif pf is not None and pf < 1.05: th_adj += 1.5
                if ev >= 0.06: th_adj -= 2.0
                elif ev < 0: th_adj += 2.0
                if dd is not None and dd >= 8: th_adj += 1.5
                elif dd is not None and dd <= 3: th_adj -= 0.8
                if wr >= 62 and cnt >= 12: th_adj -= 1.0
                elif wr < 48 and cnt >= 12: th_adj += 1.5

                if profile['strategy_mode'] == 'breakout':
                    notes.append('爆發策略模式')
                    if ev >= 0.05 and (pf or 0) >= 1.1:
                        th_adj -= 1.0
                    else:
                        th_adj += 0.8
                elif profile['strategy_mode'] == 'range':
                    notes.append('區間策略模式')
            ...
            profile['hard_block'] = hard_block
            profile['threshold_adjust'] = round(th_adj, 2)
            if hard_block:
                profile['note'] = '｜'.join(notes) if notes else 'AI封鎖'
            else:
                notes.append(f'EV/筆 {ev:+.4f}')
                if pf is not None:
                    notes.append(f'PF {pf:.2f}')
                if dd is not None:
                    notes.append(f'DD {dd:.2f}%')
...
def ai_decide_trade(sig, eff_threshold, mkt_ok, side_ok, same_dir_cnt, pos_syms, already_closing):
    ...
    rr_floor = MIN_RR_HARD_FLOOR
    if mode == 'breakout':
        rr_floor = max(rr_floor, 1.85)
    elif mode == 'range':
        rr_floor = max(1.45, rr_floor - 0.1)
    min_entry_quality = 2.0 if score >= max(ai_threshold + 4, 54) else 2.5
    if mode == 'breakout':
        min_entry_quality = max(2.5, min_entry_quality)
    base_ok = (
        score >= ai_threshold
    ...
    if profile.get('ready'):
        ...
        reasons.append('AI EV/筆 {:+.4f}'.format(float(profile.get('ev_per_trade', 0) or 0)))
        if profile.get('profit_factor') is not None:
            reasons.append('AI PF {:.2f}'.format(float(profile.get('profit_factor', 0) or 0)))
        if profile.get('max_drawdown_pct') is not None:
            reasons.append('AI DD {:.2f}%'.format(float(profile.get('max_drawdown_pct', 0) or 0)))
        reasons.append('AI模式 {}'.format(profile.get('strategy_mode', 'main')))
...
def place_order(sig):
    ...
    try:
        ...
        learn_rec={"id":trade_id,"symbol":sym,"side":side,"entry_price":sig['price'],
                   ...
                   "result":"open","post_candles":[],"missed_move_pct":None,"source":"live_bitget_v31"}
        with LEARN_LOCK:
