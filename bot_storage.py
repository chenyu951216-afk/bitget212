...
class BotStorage:
    ...
    def save_learning_state(self, db: Dict[str, Any]) -> None:
        ...
        with self._lock, self._connect() as conn:
            conn.execute('BEGIN')
            conn.execute('DELETE FROM learning_trades')
            for trade in trades:
                trade_id = str(trade.get('id') or trade.get('trade_id') or '')
                if not trade_id:
                    trade_id = f"{trade.get('symbol','unknown')}_{trade.get('entry_time','')}_{len(trades)}"
                    trade['id'] = trade_id
                conn.execute(
                    '''
                    INSERT INTO learning_trades (trade_id, symbol, result, source, entry_time, exit_time, data_json, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''',
                    (
                        trade_id,
            ...
            conn.execute('DELETE FROM learning_meta')
