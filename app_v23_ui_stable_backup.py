...
def _ai_db_stats_dict():
    out = {
        'sqlite_path': SQLITE_AI_PATH,
        'json_path': AI_DB_PATH,
        'backup_dir': SQLITE_BACKUP_DIR,
        'last_backup_time': _LAST_SQLITE_BACKUP.get('time', '--'),
        'last_backup_files': list(_LAST_SQLITE_BACKUP.get('files', [])[-6:]),
        'strategy_rows': 0,
        'learning_events': 0,
        'backtest_runs': 0,
        'latest_learning_event': None,
    }
    try:
        init_ai_sqlite()
        with _db_connect() as conn:
            ...
            out['strategy_rows'] = int(cur.fetchone()[0])
            cur.execute('SELECT COUNT(*) FROM learning_events')
            out['learning_events'] = int(cur.fetchone()[0])
            cur.execute('SELECT COUNT(*) FROM backtest_runs')
            out['backtest_runs'] = int(cur.fetchone()[0])
            cur.execute('SELECT event_time, source, symbol, regime, sample_count, win_rate, avg_pnl, ev, note FROM learning_events ORDER BY id DESC LIMIT 1')
            row = cur.fetchone()
            if row:
                out['latest_learning_event'] = {
                    'event_time': row[0], 'source': row[1], 'symbol': row[2], 'regime': row[3], 'sample_count': row[4],
                    'win_rate': row[5], 'avg_pnl': row[6], 'ev': row[7], 'note': row[8]
                }
    except Exception as e:
        out['error'] = str(e)
    return out
